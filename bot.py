...
from comandos.gatilhos import conv_handler_gatilhos
            ...
            if len(payments) > 0:
                ...
                for payment in payments:
                    ...
                    if True:
                        ...
                        # ===== NOVO: Envia evento PAID para UTMIFY =====
                        try:
                            config_data = json.loads(open('./config.json', 'r').read())
                            if config_data.get('utmify_token'):
                                from modules.tracking import UTMifyTracker
                                tracker = UTMifyTracker(config_data['utmify_token'], config_data.get('platform_name', 'NGKPay'))
                                
                                # Busca os dados salvos da UTMIFY
                                utmify_data = manager.get_payment_utmify_data(payment[1])
                                
                                if utmify_data and utmify_data.get('created_at'):
                                    # Usa a data original salva
                                    created_at = utmify_data['created_at']
                                else:
                                    # Fallback para data atual se não tiver
                                    from datetime import datetime
                                    import pytz
                                    brasilia_tz = pytz.timezone('America/Sao_Paulo')
                                    now_brasilia = datetime.now(brasilia_tz)
                                    created_at = now_brasilia.astimezone(pytz.UTC).strftime('%Y-%m-%d %H:%M:%S')
                                
                                # Envia status PAID
                                result = tracker.update_to_paid(
                                    user,
                                    bot_application.bot_data['id'],
                                    plan,
                                    payment[1],  # trans_id como order_id
                                    created_at
                                )
                                
                                if result.get('success'):
                                    print(f"UTMIFY: Pagamento confirmado enviado com sucesso")
                                else:
                                    print(f"UTMIFY: Erro ao enviar paid - {result.get('error')}")
                                    
                        except Exception as e:
                            print(f"Erro ao enviar PAID para UTMIFY: {e}")
                        # ===== FIM DO NOVO =====
...
<b>/gatilhos</b> 🧠 - Configure notificações estratégicas
...
async def pagar(update: Update, context: CallbackContext):
    ...
    gatilho_gerar_pix = gatilhos.get('gerar_pix')
    
    if gatilho_gerar_pix:
        show_alert = gatilho_gerar_pix['tipo'] == 'popup'
        await query.answer(gatilho_gerar_pix['texto'], show_alert=show_alert)
    else:
        await query.answer()
    ...
    try:
        ...
        if gate.get('type') == 'pp':
            qr_data = payment.criar_pix_pp(
                gate['token'], 
                plan['value'],
                str(query.from_user.id),  # ← NOVO: passa user_id
                context.bot_data['id']     # ← NOVO: passa bot_id
            )
            print(qr_data)
        elif gate.get('type') == 'MP':
        ...
        # ===== NOVO: Envia evento para UTMIFY quando PIX é gerado =====
        try:
            config_data = json.loads(open('./config.json', 'r').read())
            if config_data.get('utmify_token'):
                from modules.tracking import UTMifyTracker
                ...
                # Envia para UTMIFY
                result = tracker.create_waiting_payment(
                ...
                # Se enviou com sucesso, salva os dados
                if result.get('success'):
                    from datetime import datetime
                    import pytz
                    brasilia_tz = pytz.timezone('America/Sao_Paulo')
                    now_brasilia = datetime.now(brasilia_tz)
                    utc_time = now_brasilia.astimezone(pytz.UTC).strftime('%Y-%m-%d %H:%M:%S')
                    
                    manager.update_payment_utmify_order(payment_id, trans_id, utc_time)
                    print(f"UTMIFY: PIX gerado enviado com sucesso")
                else:
                    print(f"UTMIFY: Erro ao enviar - {result.get('error')}")
...
async def run_bot(token, bot_id):
    ...
    bot_application.add_handler(conv_handler_gatilhos)
