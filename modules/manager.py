...
def create_tracking_table():
    """Cria tabela para rastrear UTM e fbclid dos usuários"""
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
...
def save_user_tracking(user_id, bot_id, tracking_data):
    """Salva dados de rastreamento do usuário"""
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    ...
    cursor.execute("""
        ...
        (user_id, bot_id, fbclid, utm_source, utm_campaign, utm_medium, 
         utm_content, utm_term, sck, src, created_at, ip_address)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        user_id, bot_id,
...
def get_user_tracking(user_id, bot_id):
    """Recupera dados de rastreamento do usuário"""
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    ...
    if result:
        return {
            ...
            'fbclid': result[2],
            'utm_source': result[3],
            'utm_campaign': result[4],
            'utm_medium': result[5],
            'utm_content': result[6],
            'utm_term': result[7],
            'sck': result[8],
            'src': result[9],
...
def update_payment_utmify_order(payment_id, order_id, created_at):
    """Salva o order_id da UTMIFY e data de criação para o pagamento"""
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    ...
    if 'utmify_order_id' not in columns:
        cursor.execute("ALTER TABLE PAYMENTS ADD COLUMN utmify_order_id TEXT")
        conn.commit()
    
    if 'created_at_utc' not in columns:
        cursor.execute("ALTER TABLE PAYMENTS ADD COLUMN created_at_utc TEXT")
        conn.commit()
    
    # Atualiza o pagamento
    cursor.execute("""
        UPDATE PAYMENTS 
        SET utmify_order_id = ?, created_at_utc = ?
        WHERE id = ?
    """, (order_id, created_at, payment_id))
    
    conn.commit()
    conn.close()

def get_payment_utmify_data(trans_id):
    """Recupera dados da UTMIFY do pagamento"""
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
    ...
    if 'utmify_order_id' in columns and 'created_at_utc' in columns:
        cursor.execute("""
            SELECT utmify_order_id, created_at_utc 
            FROM PAYMENTS 
            WHERE trans_id = ?
        """, (trans_id,))
...
def update_bot_gatilhos(bot_id, gatilhos):
    """Atualiza os gatilhos mentais do bot"""
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
...
def get_bot_gatilhos(bot_id):
    """Recupera os gatilhos mentais do bot"""
    conn = sqlite3.connect("data.db")
    cursor = conn.cursor()
